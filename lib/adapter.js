...
var hop = Sequel.utils.object.hasOwnProperty;
        ...
        create: function (connectionName, collectionName, data, cb, connection) {
            ...
            function __CREATE__(connection, cb) {
                ...
                var connectionObject = connections[connectionName],
                    ...
                    autoIncCustom = utils.autoIncrementCustom(collection.definition),
                    autoIncValue = null;

                if ((!autoIncKey && !autoIncCustom) || data[autoIncKey]) {
                    __INSERT__()
                }
                else if (autoIncCustom) {
                  utils.findOrCreateCustomPk(connection, collectionName, autoIncCustom, function(err, currentPk) {
                    if (err) return cb(handleQueryError(err));
                    var newPk = utils.incrementCustomPk(currentPk, autoIncCustom);
                    autoIncValue = newPk;
                ...
                function __INSERT__() {
                    ...
                    connection.query(_query.query, function (err, result) {
                        ...
                        var values = _insertData;

                        if (autoIncValue) {
                          values.id = autoIncValue;
                        }

                        if (autoIncCustom) {
                          utils.updateCustomPk(connection, autoIncValue, autoIncCustom, collectionName, function(err) {
                            cb(err, values);
                          });
                        } else {
                          cb(err, values);
                ...
                async.eachSeries(valuesList,
                    ...
                    function (data, cb) {
                        ...
                        connection.query(_query.query, function (err, results) {
                            if (err) return cb(handleQueryError(err));
                            var value = Sequel.utils.prepareValue(data[primaryKey]);
                            value = Sequel.utils.escapeString(value, sqlOptions.escapeValue);
                            records.push(value);
                            cb();
                        });
                    },
                    ...
                    $populateBuffers: function populateBuffers(options, next) {
                        ...
                        async.auto({
                                ...
                                buildChildBuffers: ['processParent', function (results, next) {
                                    async.each(_.keys(instructions.instructions), function (population, nextPop) {
                                        ...
                                        async.eachSeries(parentRecords, function (parent, nextParent) {
                                            ...
                                            buffers.add(buffer);
                                            return async.setImmediate(nextParent);
                                        }, nextPop);
                                    }, next);
                                }],
                                ...
                                processChildren: ['buildChildBuffers', function (results, next) {
                                    ...
                                    async.eachSeries(_query.query, function (q, next) {
                                        var qs = '';
                                        var pk;
        ...
        destroy: function (connectionName, collectionName, options, cb, connection) {
            ...
            function __DESTROY__(connection, cb) {
                ...
                async.auto({
                        ...
                        destroyRecords: ['findRecords', function (recordsFound, next) {
