...
module.exports = function spawnConnection(connectionObject, fn, cb__spawnConnection) {
    ...
    if (connectionObject.connection.pool) {
        connectionObject.connection.pool.get(function (err, conn) {
            afterwards(err, conn);
        });
        return;
    }
    ...
    var conn = Firebird.attach(connectionObject.config);
    ...
    function afterwards(err, liveConnection) {
        ...
        if (err) {
            ...
            // ("could not connect")
            //
            err = new Error('Could not connect to Firebird:\n' + err.toString());
        ...
        // with the live Firebird connection (probably send a query).
