...
module.exports.configure = function (connections) {
    ...
    /**
     * Register a connection (and the collections assigned to it) with the Firebird adapter.
     *
    ...
    return function registerConnection(connection, collections, cb) {
        ...
        if (activeConnection.config.pool) {
            activeConnection.connection.pool = Firebird.pool(activeConnection.config.connectionLimit, activeConnection.config);
